...
#降低速率，默认以HT20连接保证最大兼容
	...
	if [ ! -z $bssid ] && [ ! -z $key ]
	then
		/sbin/apctrl ra0 connect "$ssid" "$key" $(echo $bssid | tr 'A-Z' 'a-z')
	else
		/sbin/apctrl ra0 connect "$ssid" "$key"
	fi
...
#自动处理CountryRegion:大于11的时候支持11-14号频道
	[ "$channel" != "auto" ] && {
	   countryregion=5
	}
...
PMFSHA256=0
LoadCodeMethod=0
AutoChannelSkipList=12;13;14;36;40;44;48;52;56;60;64;100;104;108;112;116;120;124;128;132;136;140;
...
#配置文件生成结束,重载驱动
	echo "Ralink_Global:Config file ${CFG} generated.Start interfaces now."
	drv_rt2860v2_teardown
	drv_rt2860v2_cleanup
